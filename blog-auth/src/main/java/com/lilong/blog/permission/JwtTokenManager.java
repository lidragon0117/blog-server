...
import com.lilong.blog.constants.auth.JwtClaimConstants;
...
public class JwtTokenManager implements TokenManager {
    ...
    public JwtTokenManager(SecurityProperties securityProperties, RedisService redisService) {
    ...
    private boolean validateToken(String token, boolean validateRefreshToken) {
        try {
            ...
            if (isValid) {
                ...
                if (Boolean.TRUE.equals(redisService.exists(StrUtil.format(RedisConstants.Auth.BLACKLIST_TOKEN, jti)))) {
                    return false;
                }
            }
            return isValid;
    ...
    public void invalidateToken(String token) {
        ...
        if (expirationAt != null) {
            ...
            redisService.set(blacklistTokenKey, null, expirationIn);
        } else {
            // 永不过期的Token永久加入黑名单
            redisService.set(blacklistTokenKey, null);
        }
        ;
    }
