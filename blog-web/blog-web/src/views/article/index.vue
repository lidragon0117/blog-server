    ...
    <div class="content-layout" id="articleBox" :class="{ center: !showSidebar }">
      <main class="article-main">
        ...
        <article class="article-content">
          <!-- 免费内容 -->
          <div v-if="article.readType === 1" class="markdown-body">
            <mavon-editor
  ...
  methods: {
    ...
    async getArticle() {
      ...
      try {
        ...
        console.log('文章详情返回数据:', res.data)
        console.log('后端返回的readType:', res.data.readType)

        // 如果readType是0、undefined或null，默认设置为1（免费阅读）
        const readTypeValue = res.data.readType
        if (readTypeValue === 0 || readTypeValue === undefined || readTypeValue === null) {
          res.data.readType = 1
        }
        ...
        console.log('处理后article.readType:', this.article.readType)
        console.log('处理后article.category:', this.article.category)
        console.log('contentMd长度:', this.article.contentMd?.length)
        ...
        // 计算阅读时间
        const textContent = this.article.contentMd || ''
        this.readTime = Math.ceil(textContent.length / 300)
     ...
     */
    generateToc() {
      // mavon-editor 的内容在 .markdown-body .v-note-wrapper 内
      const headings = document.querySelectorAll('.markdown-body .v-note-wrapper h1,.markdown-body .v-note-wrapper h2,.markdown-body .v-note-wrapper h3,.markdown-body .v-note-wrapper h4,.markdown-body .v-note-wrapper h5,.markdown-body .v-note-wrapper h6')

      this.tocItems = Array.from(headings).map(heading => {
        // 获取标题文本
        let text = heading.textContent.trim()

        // 如果标题已有ID，使用它；否则根据文本生成ID
        let id = heading.id
        if (!id) {
      ...
      }).filter(item => item.text.length > 0) // 过滤掉空标题
    },
    /**
     * 点赞
     */
    ...
    shareToWeibo() {
      const url = encodeURIComponent(this.currentUrl)
      const title = encodeURIComponent(`${this.article.title} - 智能通用`)
      window.open(
